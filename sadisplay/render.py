...
env = Environment(loader=PackageLoader("sadisplay", "templates"))
...
def plantuml(desc):
    ...
    for cls in classes:
        ...
        # class indexes
        class_desc += [
            (_clean(format_index_type_string(i["cols"])), format_index(i["name"]))
...
def dot(desc, schema_subgraphs=True):
    ...
    for key, classes in subgraphs.items():
        ...
        for cls in classes:
            ...
            props = " ".join(
                [
                    ix_template.render(name=format_property(p), type="PROPERTY")
                    for p in cls["props"]
                ]
            )
            methods = " ".join(
                [ix_template.render(name=m, type="METHOD") for m in cls["methods"]]
            )
            ...
            # Raise a warning
            warn("Indexes are disabled in this version of sadisplay.")
